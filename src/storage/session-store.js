...
class SessionStore {
    ...
    endSession(sessionId) {
        ...
        if (totalCache > 0) {
            session.metadata.cacheHitRate = Math.round(
                (session.metrics.cacheHits / totalCache) * 100
            );
        }
    ...
    recordSttMetric(sessionId, latencyMs) {
        ...
        session.metrics = session.metrics || { sttLatencies: [], llmLatencies: [], cacheHits: 0, cacheMisses: 0 };
    ...
    recordLlmMetric(sessionId, latencyMs, cached = false, questionType = 'general') {
        ...
        session.metadata.questionTypes = session.metadata.questionTypes || { technical: 0, experience: 0, general: 0 };
    ...
    filterSessions(filters = {}) {
        ...
        if (filters.minDuration) {
            sessions = sessions.filter(s =>
                (s.metadata?.duration || 0) >= filters.minDuration
            );
        }

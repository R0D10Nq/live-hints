...
class OnboardingController {
  ...
  bindEvents() {
    ...
    // File remove buttons
    document
      .getElementById('resume-remove')
      ?.addEventListener('click', () => this.removeFile('resume'));
    document
      .getElementById('vacancy-remove')
      ?.addEventListener('click', () => this.removeFile('vacancy'));

    // File replace buttons
    document
      .getElementById('resume-replace')
      ?.addEventListener('click', () => this.replaceFile('resume'));
    document
      .getElementById('vacancy-replace')
      ?.addEventListener('click', () => this.replaceFile('vacancy'));

    // Keyboard navigation — игнорируем внутри textarea и input чтобы не ломать ввод текста
  ...
  async startMicrophoneTest() {
    try {
      ...
      document.getElementById('btn-test-mic').innerHTML =
        '<span class="btn-icon">■</span> Остановить';
  ...
  stopMicrophoneTest() {
    ...
    document.getElementById('btn-test-mic').innerHTML =
      '<span class="btn-icon">◉</span> Тест микрофона';
    ...
    if (this.selectedMode === 'job_interview_ru') {
      ...
      if (subtitle)
        subtitle.textContent =
          'AI будет опираться на ваш реальный опыт при формулировке ответов. Обязательно для собеседований.';
    } else {
      // Для других режимов можно пропустить
      if (skipBtn) skipBtn.classList.remove('hidden');
      if (subtitle)
        subtitle.textContent =
          'AI будет опираться на ваш реальный опыт при формулировке ответов. Опционально для этого режима.';
    }
  }
  ...
  async finish() {
    const customContext = document.getElementById('mode-context')?.value || '';
    const settings = {

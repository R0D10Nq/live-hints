...
export class UIController {
    ...
    bindUIEvents() {
        ...
        this.elements.btnAdvancedMode?.addEventListener('click', () => this.setSettingsMode('advanced'));
    ...
    showToast(message, type) { this.utils.showToast(message, type); }
    showError(message) { this.utils.showError(message); }
    hideError() { this.utils.hideError(); }

    addTranscriptItem(text, timestamp, source) { this.transcriptUI.addItem(text, timestamp, source); }
    addHintItem(text, timestamp, latencyMs) { this.hintsUI.addHintItem(text, timestamp, latencyMs); }

    showHintLoading() { this.hintsUI.showLoading(); }
    hideHintLoading() { this.hintsUI.hideLoading(); }
    showHintsEmptyState() { this.hintsUI.showEmptyState(); }
    hideHintsEmptyState() { this.hintsUI.hideEmptyState(); }

    createStreamingHintElement() { return this.hintsUI.createStreamingElement(); }
    updateStreamingHint(el, text) { this.hintsUI.updateStreamingHint(el, text); }
    finalizeStreamingHint(el, text, opts) { this.hintsUI.finalizeStreamingHint(el, text, opts); }

    showPrevHint() { this.hintsUI.showPrevHint(); }
    showNextHint() { this.hintsUI.showNextHint(); }
    goToLastHint() { this.hintsUI.goToLastHint(); }
    updatePaginationButtons() { this.hintsUI.updatePaginationButtons(); }

    showHistoryModal() { this.modals.showHistory(); }
    hideHistoryModal() { this.modals.hideHistory(); }
    showSessionView(id) { this.modals.showSessionView(id); }
    hideSessionView() { this.modals.hideSessionView(); }
    showHelpModal() { this.modals.showHelp(); }
    hideHelpModal() { this.modals.hideHelp(); }
    renderSessionsList() { this.modals.renderSessionsList(); }

    toggleTranscriptSidebar() { this.transcriptUI.toggle(); }
    toggleTranscriptsVisibility() { this.transcriptUI.toggle(); }
    restoreTranscriptState() { this.transcriptUI.restoreState(); }
    ...
    clearHints() { this.hintsUI.clear(); }
    clearTranscript() { this.transcriptUI.clear(); }

    getTranscriptText() { return this.transcriptUI.getText(); }
    getHintsText() { return this.hintsUI.getHintsText(); }

    // Backwards compatibility getters
    get hints() { return this.hintsUI.hints; }
    set hints(value) { this.hintsUI.hints = value; }
    get currentHintIndex() { return this.hintsUI.currentHintIndex; }
    set currentHintIndex(value) { this.hintsUI.currentHintIndex = value; }
    get lastTranscriptText() { return this.transcriptUI.lastTranscriptText; }
    get lastHintText() { return this.hintsUI.lastHintText; }
    ...
    // Utilities - delegated
    formatTime(timestamp) { return this.utils.formatTime(timestamp); }
    formatLatency(latencyMs) { return this.utils.formatLatency(latencyMs); }
    escapeHtml(text) { return this.utils.escapeHtml(text); }
    renderMarkdown(text) { return this.utils.renderMarkdown(text); }
}
